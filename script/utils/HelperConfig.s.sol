...
contract HelperConfig is Script {
    ...
    function getConfigByChainId(
        ...
        if (chainId == LOCAL_CHAIN_ID) {
            ...
            console.log(
                "Invalid account %s for chainId: %s",
                networkConfigs[chainId].backendSigner,
                chainId
            );
    ...
    function getTokenBySymbol(
        ...
        for (uint256 i = 0; i < config.tokensConfig.length; i++) {
            if (
                keccak256(bytes(config.tokensConfig[i].symbol)) ==
